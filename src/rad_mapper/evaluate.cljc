...
(def bi-macs "'macs' is a misnomer, since they are rewritten for SCI to functions. They are treated like bi-vars below."
...
(def ctx (sci/init {:namespaces {'bi builtins-ns}}))

(defn ctx-nspaces
  "Walk form replacing namespace 'rad-mapper.builtins' with 'bi'."
  [form]
  (let [ns-alia {"rad-mapper.builtins" "rad-mapper.builtins"
                 "bi"                  "rad-mapper.builtins"}]
    (letfn [(ni [form]
  ...
  (binding [*ns* (find-ns 'user)]
    (try
      ...
      (let [res #?(:clj  (if use-sci?
                           (sci/eval-string* ctx (-> form ctx-nspaces str))
                           (-> form str util/read-str eval))
                   :cljs (sci/eval-string* ctx (-> form ctx-nspaces str)))]
